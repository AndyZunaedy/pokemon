<template>
    ...
    <main>
        ...
        <section class="container py-5">
            ...
            <div class="row mb-4" v-if="!displayLoading && pokemonDataType.length > 0">
             ...
             <div class="row mb-3" v-if="!displayLoading && pokemonDataType.length > 0">
                <div class="col-12">
                    <p class="text-muted">
                 ...
                 <template v-else-if="searchQuery && filteredPokemonData.length === 0">
                    <div class="col-12">
                        <div class="alert alert-info text-center">
                            ...
                            <p>No Pokemon matching "{{ searchQuery }}"</p>
                        </div>
                    </div>
                </template>

                <template v-else-if="!displayLoading && pokemonDataType.length === 0">
                    <div class="col-12">
                        <div class="alert alert-warning text-center">
                            <i class="fas fa-info-circle fa-2x mb-3"></i>
                            <h5>No Pokemon Data</h5>
                            <p>Loading Pokemon data...</p>
                        </div>
                    </div>
                </template>
            </div>
            <div class="row mt-4" v-if="!displayLoading && totalPages > 1">
                <div class="col-12">
            ...
            <div class="row mt-3" v-if="!displayLoading && pokemonDataType.length > 0">
...
import { FetchingAllPokemon } from '@/store/getPokemon.js';
import Header from '@/components/Header.vue';
import Footer from '@/components/Footer.vue';

const route = useRoute();
const pokemonAllStore = FetchingAllPokemon();

const searchQuery = ref('');
const currentPage = ref(1);
const itemsPerPage = ref(12);

const selectedType = computed(() =>  null);
const pageTitle = computed(() => `All Pokemon (${pokemonAllStore.totalCount || 0})`);

const pokemonDataType = computed(() => pokemonAllStore.pokemonAllData || []);
const isLoadingType = computed(() => pokemonAllStore.loading);
...
const loadingProgress = computed(() => {
    if (!isLoadingType.value) return 100;
    
    const currentCount = pokemonDataType.value.length;
    const totalExpected = pokemonAllStore.totalCount || 1500; // Estimasi
    return Math.min(100, Math.round((currentCount / totalExpected) * 100));
});
...
const fetchAllPokemon = () => {
    pokemonAllStore.fetchAllPokemon();
};
